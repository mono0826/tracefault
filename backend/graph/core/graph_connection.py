...
_connection_manager: "GraphConnectionManager" = None


def get_connection_manager() -> "GraphConnectionManager":
    """获取全局连接管理器实例（懒加载）"""
    global _connection_manager
    if _connection_manager is None:
        _connection_manager = GraphConnectionManager()
    return _connection_manager


class _ConnectionManagerProxy:
    """延迟加载代理，保持 connection_manager.xxx() 调用兼容"""
    def __getattr__(self, name):
        return getattr(get_connection_manager(), name)


connection_manager = _ConnectionManagerProxy()