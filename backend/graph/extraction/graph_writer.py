...
class GraphWriter:
    ...
    def process_and_write_graph_documents(self, file_contents: List[Tuple[str, List[Chunk], List[str]]]) -> None:
        """
        并行转换 GraphDocument 并批量写入 Neo4j

        Args:
            file_contents: [(file_name, [Chunk, ...], [LLM结果, ...]), ...]
        """
        all_graph_documents = []
        all_chunk_ids = []
        ...
        print(f"开始处理 {total_chunks} 个 chunks 的 GraphDocument")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_index):
                ...
                try:
                    ...
                    error_count += 1
                    print(f"处理 chunk 时出错 (已有 {error_count} 个错误): {e}")
                    all_graph_documents[idx] = None
                    all_chunk_ids[idx] = None

        all_graph_documents = [doc for doc in all_graph_documents if doc is not None]
        all_chunk_ids = [cid for cid in all_chunk_ids if cid is not None]

        print(f"共处理 {total_chunks} 个 chunks, 有效文档 {len(all_graph_documents)}, 错误 {error_count}")
