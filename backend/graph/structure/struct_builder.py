...
from backend.graph.core import connection_manager, generate_hash
from backend.config.settings import BATCH_SIZE as DEFAULT_BATCH_SIZE
from backend.config.settings import MAX_WORKERS as DEFAULT_MAX_WORKERS
...
class GraphStructureBuilder:
    ...
    def create_relation_between_chunks(self, file_name: str, chunks: List[Chunk]) -> List[Dict]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_document = Document(page_content=chunk.content, metadata=metadata)
            ...
            # 创建关系数据
            if firstChunk:
                relationships.append({"type": "FIRST_CHUNK", "chunk_id": chunk_id})
            else:
    ...
    def parallel_process_chunks(self, file_name: str, chunks: List[Chunk], max_workers=None) -> List[Dict]:
        ...
        def process_chunk_batch(batch, start_index):
            ...
            if start_index > 0 and start_index < len(chunks):
                ...
                current_chunk_id = prev_chunk.chunk_id or generate_hash(prev_chunk.content)
                # 计算前面所有chunk的offset
                for j in range(start_index):
                    offset += len(chunks[j].content)
        ...
        for i in range(0, len(all_batch_data), db_batch_size):
            ...
            self._create_chunks_and_relationships(batch[0]['f_name'], batch, rel_batch)
