...
class ChineseTextChunker:
    ...
    def chunk_document(self, doc: Document) -> List[Chunk]:
        """处理单个 Document，返回 Chunk 列表

        Args:
            doc: 文档对象（来自 FileReader）

        Returns:
            List[Chunk]: 分块结果，chunk_id 由内容 SHA1 自动生成，
                         doc_id 为文档内容的 SHA256
        """
    ...
    def chunk_documents(self, docs: List[Document]) -> List[Chunk]:
        """批量处理多个 Document，返回扁平化的 Chunk 列表
        ...
        for doc in docs:
            if doc.status == "failed":
                continue
            results.extend(self.chunk_document(doc))
        return results
