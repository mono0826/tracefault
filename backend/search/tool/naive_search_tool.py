...
from backend.config.prompts import NAIVE_PROMPT, NAIVE_SEARCH_QUERY_PROMPT
from backend.config.settings import response_type, naive_description, NAIVE_SEARCH_TOP_K, PROJECT_ROOT
...
class NaiveSearchTool(BaseSearchTool):
    ...
    def __init__(self):
        """初始化Naive搜索工具"""
        # 调用父类构造函数
        super().__init__(cache_dir=str(PROJECT_ROOT / ".cache/naive_search"))
    ...
    def search(self, query_input: Any) -> str:
        ...
        try:
            ...
            RETURN c.id AS id, c.text AS text, c.fileName AS file_name, c.embedding AS embedding
            LIMIT 100  // 获取候选集
            """)
            ...
            for item in results:
                ...
                if text:
                    ...
                    chunks_content.append(f"Chunk ID: {chunk_id}{file_info}\n{text}")
                    chunk_refs.append({"id": chunk_id, "file_name": file_name})
            ...
            if "{'data': {'Chunks':" not in answer:
                # 添加引用信息
                chunk_refs_json = ", ".join([f"{{'id':'{r['id']}', 'file_name':'{r['file_name']}'}}" for r in chunk_refs[:5]])
                answer += f"\n\n{{'data': {{'Chunks':[{chunk_refs_json}] }} }}"
